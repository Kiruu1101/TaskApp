...
const CreateEditTask = ({ onCloseModal, taskToEdit }) => {
  ...
  useEffect(() => {
    if (taskToEdit && taskToEdit.assignee) {
      setAssignee(taskToEdit.assignee);
      setSelectedAssignee(taskToEdit.assignee);
    }
  }, [taskToEdit]);
  ...
  const handleDoneChange = (id, e) => {
    const newArray = checkLists.map((checkList) =>
      checkList._id === id ? { ...checkList, done: e.target.checked } : checkList
    );
  ...
  const assignEmail = (email) => {
    ...
    setAssigneeListOpen(false); // Close the dropdown after selection
  };
  ...
  const onCreateEdit = async () => {
    ...
    if (checkLists.length === 0) {
      setError({
        ...initialErrorState,
        checklistArr: "Create at least one subtask",
      });
